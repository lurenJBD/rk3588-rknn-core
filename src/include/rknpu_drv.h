/* SPDX-License-Identifier: GPL-2.0 */
/*
 * Copyright (C) Rockchip Electronics Co., Ltd.
 * Author: Felix Zeng <felix.zeng@rock-chips.com>
 * Author: lurenJBD <31967654+lurenJBD@users.noreply.github.com>
 */

#ifndef __LINUX_RKNPU_DRV_H_
#define __LINUX_RKNPU_DRV_H_

#include <linux/completion.h>
#include <linux/device.h>
#include <linux/kref.h>
#include <linux/irq.h>
#include <linux/iommu.h>
#include <linux/xarray.h>
#include <linux/platform_device.h>
#include <linux/spinlock.h>
#include <linux/mutex.h>
#include <linux/regulator/consumer.h>
#include <linux/version.h>
#include <linux/hrtimer.h>
#include <linux/idr.h>
#include <linux/miscdevice.h>

#include "rknpu_compat.h"

#include <drm/drm_mm.h>

#include "rknpu_job.h"
#include "rknpu_fence.h"
#include "rknpu_debugger.h"
#include "rknpu_mm.h"

#define DRIVER_NAME "rknpu"
#define DRIVER_DESC "RKNPU driver"
#define DRIVER_DATE "20240828"
#define DRIVER_MAJOR 0
#define DRIVER_MINOR 9
#define DRIVER_PATCHLEVEL 8

#define LOG_TAG "RKNPU"

/* sample interval: 1000ms */
#define RKNPU_LOAD_INTERVAL 1000000000

/*
 * Upper bound on the IOMMU domain IDs a submit may name.
 * Domains & drm_mm pools are allocated lazily.
 */
#define RKNPU_MAX_IOMMU_DOMAIN_NUM 64
/* Domains 0..15 reserved for legacy / explicit multi-domain models; 16..63 for per-FD isolation */
#define RKNPU_PER_FD_DOMAIN_START 16

#define LOG_INFO(fmt, args...) pr_info(LOG_TAG ": " fmt, ##args)
#define LOG_WARN(fmt, args...) pr_warn(LOG_TAG ": " fmt, ##args)
#define LOG_DEBUG(fmt, args...) pr_devel(LOG_TAG ": " fmt, ##args)
#define LOG_ERROR(fmt, args...) pr_err(LOG_TAG ": " fmt, ##args)

#define LOG_DEV_INFO(dev, fmt, args...) dev_info(dev, LOG_TAG ": " fmt, ##args)
#define LOG_DEV_WARN(dev, fmt, args...) dev_warn(dev, LOG_TAG ": " fmt, ##args)
#define LOG_DEV_DEBUG(dev, fmt, args...) dev_dbg(dev, LOG_TAG ": " fmt, ##args)
#define LOG_DEV_ERROR(dev, fmt, args...) dev_err(dev, LOG_TAG ": " fmt, ##args)

/*
 * Verbose, per-operation logs (per-submit, per-IRQ, per-buffer, per-arm).
 * Compiled in, but gated at runtime by the rknpu_debug_log module parameter so
 * a full model load does not emit ~120k printk lines on the hot path. When the
 * switch is off the cost is a single predicted-not-taken branch, and the format
 * arguments are not evaluated. Errors, warnings and once-per-probe lines stay
 * on the unconditional LOG_* macros above. Toggle live via
 * /sys/module/rknpu/parameters/rknpu_debug_log.
 */
extern bool rknpu_debug_log;
#define LOG_DBG(fmt, args...)                                                \
	do {                                                                 \
		if (unlikely(rknpu_debug_log))                               \
			pr_info(LOG_TAG ": " fmt, ##args);                   \
	} while (0)
#define LOG_DEV_DBG(dev, fmt, args...)                                       \
	do {                                                                 \
		if (unlikely(rknpu_debug_log))                               \
			dev_info(dev, LOG_TAG ": " fmt, ##args);             \
	} while (0)


#define RKNPU_CACHE_SG_TABLE_NUM 2

enum rknpu_core_mmio_reg {
	RKNPU_CORE_REG_VERSION,
	RKNPU_CORE_REG_VERSION_NUM,
	RKNPU_CORE_REG_PC_OP_EN,
	RKNPU_CORE_REG_PC_DATA_ADDR,
	RKNPU_CORE_REG_PC_DATA_AMOUNT,
	RKNPU_CORE_REG_INT_MASK,
	RKNPU_CORE_REG_INT_CLEAR,
	RKNPU_CORE_REG_INT_STATUS,
	RKNPU_CORE_REG_INT_RAW_STATUS,
	RKNPU_CORE_REG_PC_TASK_CONTROL,
	RKNPU_CORE_REG_PC_DMA_BASE_ADDR,
	RKNPU_CORE_REG_PC_TASK_STATUS_RK3588,
	RKNPU_CORE_REG_MULTICORE_CFG,
	RKNPU_CORE_REG_MULTICORE_CFG2,
	RKNPU_CORE_REG_COUNT,
};

struct rknpu_irqs_data {
	const char *name;
	irqreturn_t (*irq_hdl)(int irq, void *ctx);
};

struct rknpu_amount_data {
	uint16_t offset_clr_all;
	uint16_t offset_dt_wr;
	uint16_t offset_dt_rd;
	uint16_t offset_wt_rd;
};

struct rknpu_config {
	__u32 bw_priority_addr;
	__u32 bw_priority_length;
	__u64 dma_mask;
	__u32 pc_data_amount_scale;
	__u32 pc_task_number_bits;
	__u32 pc_task_number_mask;
	__u32 pc_task_status_offset;
	__u32 pc_dma_ctrl;
	const struct rknpu_irqs_data *irqs;
	int num_irqs;
	__u64 nbuf_phyaddr;
	__u64 nbuf_size;
	__u64 max_submit_number;
	__u32 core_mask;
	const struct rknpu_amount_data *amount_top;
	const struct rknpu_amount_data *amount_core;
	void (*state_init)(struct rknpu_device *rknpu_dev);
	int (*cache_sgt_init)(struct rknpu_device *rknpu_dev);
};

struct rknpu_timer {
	ktime_t busy_time;
	ktime_t total_busy_time;
};

struct rknpu_subcore_data {
	struct list_head todo_list;
	wait_queue_head_t job_done_wq;
	struct rknpu_job *job;
	int64_t task_num;
	struct rknpu_timer timer;
};

/**
 * RKNPU device
 *
 * @base: IO mapped base address for device
 * @dev: Device instance
 * @drm_dev: DRM device instance
 */
struct rknpu_core;

/*
 * One IOVA allocator per domain ID. Each domain ID owns an independent
 * window. Aliasing addresses are safe because a core attaches at most one
 * domain at a time (serialized by iommu_active_domain / iommu_active_refcount).
 */
struct rknpu_iova_pool {
	struct drm_mm mm;
	bool ready;
	u64 begin; /* pool window start (PAGE_SIZE) */
	u64 end;   /* pool window end (aperture_end) */
};

struct rknpu_device {
	void __iomem *base[RKNPU_MAX_CORES];
	void __iomem *multicore_base[RKNPU_MAX_CORES][2];
	struct rknpu_core *cores;
	unsigned int available_cores;
	struct device *dev;
	struct drm_device *drm_dev;
	/*
	 * librknnrt uses MEM_CREATE's obj_addr (canonical dma_addr here) as
	 * the token in MEM_SYNC and SUBMIT. This is a lookup aid only; all
	 * operations still validate that the object is reachable from the
	 * caller's drm_file handle table.
	 */
	struct xarray gem_dma_xa;
	atomic_t sequence;
	spinlock_t lock;
	spinlock_t irq_lock;
	struct mutex power_lock;
	struct mutex reset_lock;
	bool reset_failed;
	bool shutting_down;
	bool facade_ready;
	unsigned int commit_count;
	wait_queue_head_t commit_wq;
	struct list_head jobs;
	struct workqueue_struct *job_wq;
	struct rknpu_subcore_data subcore_datas[RKNPU_MAX_CORES];
	const struct rknpu_config *config;
	void __iomem *bw_priority_base;
	struct rknpu_fence_context *fence_ctx;
	bool iommu_en;
	/*
	 * [core][domain_id]: each RKNN core owns an IOMMU domain per domain ID.
	 * Lazily allocated from cores[core].dev when the domain is first mapped.
	 */
	struct iommu_domain *iommu_domains[RKNPU_MAX_CORES]
					  [RKNPU_MAX_IOMMU_DOMAIN_NUM];
	/*
	 * Allocator pool per domain ID, shared across all cores so that any
	 * buffer is translatable by every core at the same IOVA.
	 */
	struct rknpu_iova_pool iommu_pools[RKNPU_MAX_IOMMU_DOMAIN_NUM];
	struct mutex iommu_shared_mm_lock;
	bool iommu_shared_mm_ready;
	u64 iommu_shared_mm_begin;
	u64 iommu_shared_mm_end;
	/*
	 * Per-core attach serialization. An iommu_group attaches to one domain
	 * at a time. iommu_active_refcount tracks active jobs on the domain;
	 * a job requesting a different domain waits for the refcount to drain.
	 */
	int iommu_active_domain[RKNPU_MAX_CORES];
	unsigned int iommu_active_refcount[RKNPU_MAX_CORES];
	wait_queue_head_t iommu_active_wq[RKNPU_MAX_CORES];
	/*
	 * Live IOVA accounting for the shared allocator, updated under
	 * iommu_shared_mm_lock. Observability only: lets a real model load
	 * show whether reserved IOVA is a genuine resident working set or a
	 * lazy-destroy leak, and reports window occupancy at -ENOSPC.
	 */
	u64 iommu_live_bytes;
	u64 iommu_live_objs;
	u64 iommu_peak_bytes;
	u64 iommu_peak_objs;
	u64 iommu_total_reserves;
	u64 iommu_total_releases;
	/* Per-domain IOVA accounting counters, updated under iommu_shared_mm_lock */
	u64 iommu_domain_live_bytes[RKNPU_MAX_IOMMU_DOMAIN_NUM];
	u64 iommu_domain_peak_bytes[RKNPU_MAX_IOMMU_DOMAIN_NUM];
	u64 iommu_domain_live_objs[RKNPU_MAX_IOMMU_DOMAIN_NUM];
	struct mutex iommu_domain_lock;
	struct reset_control **srsts;
	int num_srsts;
	struct clk_bulk_data *clks;
	int num_clks;
	struct regulator *vdd;
	struct regulator *mem;
	struct monitor_dev_info *mdev_info;
	struct ipa_power_model_data *model_data;
	struct thermal_cooling_device *devfreq_cooling;
	struct devfreq *devfreq;
	unsigned long ondemand_freq;
	struct rockchip_opp_info opp_info;
	unsigned long current_freq;
	unsigned long current_volt;
	int bypass_soft_reset;
	bool soft_reseting;
	atomic_t power_refcount;
	atomic_t cmdline_power_refcount;
	struct delayed_work power_off_work;
	struct workqueue_struct *power_off_wq;
	struct rknpu_debugger debugger;
	struct hrtimer timer;
	ktime_t kt;
	phys_addr_t sram_start;
	phys_addr_t sram_end;
	phys_addr_t nbuf_start;
	phys_addr_t nbuf_end;
	uint32_t sram_size;
	uint32_t nbuf_size;
	void __iomem *sram_base_io;
	void __iomem *nbuf_base_io;
	struct rknpu_mm *sram_mm;
	unsigned long power_put_delay;
	bool reserved_mem_initialized;
	int iommu_domain_id;
	struct ida domain_ida;
	struct sg_table *cache_sgt[RKNPU_CACHE_SG_TABLE_NUM];
};

struct rknpu_file_priv {
	struct drm_file *file_priv;
	int domain_id;
};

struct rknpu_session {
	struct rknpu_device *rknpu_dev;
	struct list_head list;
};

u32 rknpu_core_read(struct rknpu_device *rknpu_dev, unsigned int core,
		    enum rknpu_core_mmio_reg reg);
void rknpu_core_write(struct rknpu_device *rknpu_dev, unsigned int core,
		      enum rknpu_core_mmio_reg reg, u32 value);
u32 rknpu_core_read_dynamic(struct rknpu_device *rknpu_dev, unsigned int core,
			    resource_size_t offset);
void rknpu_core_write_dynamic(struct rknpu_device *rknpu_dev, unsigned int core,
			      resource_size_t offset, u32 value);

int rknpu_power_get(struct rknpu_device *rknpu_dev);
int rknpu_power_put(struct rknpu_device *rknpu_dev);
int rknpu_power_put_delay(struct rknpu_device *rknpu_dev);

#endif /* __LINUX_RKNPU_DRV_H_ */
